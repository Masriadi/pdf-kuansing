            ...
            <Card variant="default" className="flex-1 p-6 rounded-[2rem] border border-white/20 dark:border-zinc-800/40 bg-white/40 dark:bg-black/30 backdrop-blur-md flex flex-col justify-between shadow-xl space-y-6">
              ...
              <div className="space-y-4 flex-1">
                <div className="border-b border-[hsl(var(--color-border))] pb-3">
                  <h3 className="text-base font-bold text-[hsl(var(--color-foreground))] flex items-center gap-2">
                    ...
                    {tTools('ocrPdf.configTitle') || 'OCR Configuration'}
                  </h3>
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-[hsl(var(--color-muted-foreground))] uppercase tracking-wider flex items-center gap-1.5">
                    <Languages className="w-4 h-4" /> {tTools('ocrPdf.languageLabel') || 'Select Recognition Language (Multi-select)'}
                  </label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-[hsl(var(--color-muted-foreground))] uppercase tracking-wider mb-2">
                      {tTools('ocrPdf.outputFormatLabel') || 'Output File Format'}
                    </label>
                    <Select
                      value={outputFormat}
                      ...
                      disabled={isProcessing}
                    >
                      <option value="searchable-pdf">{tTools('ocrPdf.formatSearchablePdf') || 'Searchable PDF'}</option>
                      <option value="text">{tTools('ocrPdf.formatTextFile') || 'Extract Text File (.txt)'}</option>
                    </Select>
                  </div>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-[hsl(var(--color-muted-foreground))] uppercase tracking-wider mb-2">
                      {tTools('ocrPdf.scaleLabel') || 'Recognition Resolution'}
                    </label>
                    <Select
                      value={scale}
                      ...
                      disabled={isProcessing}
                    >
                      <option value="1">{tTools('ocrPdf.scaleLow') || 'Standard (for clear documents)'}</option>
                      <option value="2">{tTools('ocrPdf.scaleMedium') || 'HD (Recommended, more stable)'}</option>
                      <option value="3">{tTools('ocrPdf.scaleHigh') || 'Ultra HD (Slower, for blurry documents)'}</option>
                    </Select>
                  </div>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-[hsl(var(--color-muted-foreground))] uppercase tracking-wider mb-2">
                      {tTools('ocrPdf.pageRangeLabel') || 'Specify Page Range'}
                    </label>
                    <input
                      type="text"
                      value={pageRange}
                      onChange={(e) => setPageRange(e.target.value)}
                      placeholder={tTools('ocrPdf.pageRangePlaceholderShort') || 'e.g.: 1-3, 5'}
                      disabled={isProcessing}
              ...
              <div className="pt-4 border-t border-[hsl(var(--color-border))] mt-6">
                ...
                >
                  ...
                  {tTools('ocrPdf.startButton') || 'Start OCR'}
                </Button>
              </div>
            ...
            <Card variant="outlined" className="flex-1 p-6 bg-zinc-950 border-2 border-dashed border-[hsl(var(--color-border))] rounded-[2rem] flex flex-col items-center justify-center relative overflow-hidden shadow-inner h-full min-h-[380px]">
              ...
              <div className="w-full mt-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl p-4 z-20 space-y-1.5">
                <h4 className="text-[10px] font-black tracking-widest text-purple-400 uppercase flex items-center gap-1.5">
                  ...
                  {tTools('ocrPdf.infoTitle2') || 'Core Advantage of Dual-layer PDF'}:
                </h4>
                <p className="text-[10px] text-zinc-400 leading-relaxed pl-1">
                  {tTools('ocrPdf.infoDescription2') || 'The reconstructed searchable PDF feature calculates OCR text layout physical bounding boxes in the background and precisely injects a fully transparent text layer on top of the original PDF layer. This not only preserves the appearance of the original scanned document but also perfectly supports text selection, copying, searching, and retrieval.'}
                </p>
              </div>
      ...
      {status === 'complete' && result && (
        <Card variant="default" className="p-8 rounded-[2.5rem] bg-white/40 dark:bg-black/30 backdrop-blur-md border border-white/20 dark:border-zinc-800/40 text-center space-y-6 shadow-2xl">
          ...
          <div className="space-y-2 max-w-sm mx-auto">
            <h3 className="text-base font-extrabold text-[hsl(var(--color-foreground))]">{tTools('ocrPdf.completeTitle') || 'Text Recognition Complete!'}</h3>
            <p className="text-xs text-[hsl(var(--color-muted-foreground))]">
              {outputFormat === 'searchable-pdf' 
                ? (tTools('ocrPdf.completeSearchableDesc') || 'Searchable PDF compilation complete. The original scanned image has been overlaid with a text mask. You can open the file with any PDF viewer and perform text selection, search, and copy.')
                : (tTools('ocrPdf.completeTextDesc') || 'Text file structure extraction successful. Text stream preview is displayed below.')
              }
            </p>
          </div>
      ...
      {textPreview && (
        <Card variant="outlined" size="lg" className="rounded-3xl shadow-sm">
          <h3 className="text-sm font-bold text-[hsl(var(--color-foreground))] mb-4">
            {tTools('ocrPdf.previewTitle2') || 'Extracted Text Stream Preview'}
          </h3>
